...
#refernce of testvideo.mp4& testvideo2 below
...
blopdetector = cv2.SimpleBlobDetector_create(params)

# capture object to read the frame from the video
capture = cv2.VideoCapture("Frame/testvideo2.mp4")
# TrackingTwoPeople.avi  #regionOfInterest = frame[0:700, 15:1280]

# start loop to extract thr frames one after another. on each loop we take one frame

# object detection from stable camera
# is going to extract the moving object from the camera
object_detector = cv2.createBackgroundSubtractorMOG2(history=100, varThreshold=25)# find the best range threshold 50 is ok
...
while True:
    ...
    regionOfInterest = frame[0:800, 15:1280]
    # Detect blobs.
    ...
    key_points = blopdetector.detect(frame)
